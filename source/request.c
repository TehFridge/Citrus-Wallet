...
bool refresh_data(const char *url, const char *data, struct curl_slist *headers, ResponseBuffer *response_buf, bool post) {
...
void request_worker(void* arg) {
    ...
    while (request_running) {
        ...
        refresh_data(req.url, req.data, req.headers, buf, false);
