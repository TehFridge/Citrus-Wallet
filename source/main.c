...
int main(int argc, char* argv[]) {
	...
	if (access("/3ds/CTR_WALLET_CODES.json", F_OK) == 0){
		// json_t *jsonfl = json_load_file("/3ds/CTR_WALLET_CODES.json", 0, NULL);
		...
		// Entry 1: "dupa"
		json_t *dupa_obj = json_object();
		json_object_set_new(dupa_obj, "symbology", json_string("CODE128"));
		json_object_set_new(dupa_obj, "data", json_string("1234567890"));
		json_object_set_new(root, "dupa", dupa_obj);

		// Entry 2: "lol"
		json_t *lol_obj = json_object();
		json_object_set_new(lol_obj, "symbology", json_string("QR"));
		json_object_set_new(lol_obj, "data", json_string("https://example.com"));
		json_object_set_new(root, "lol", lol_obj);

		// Save to file
		json_dump_file(root, "/3ds/CTR_WALLET_CODES.json", JSON_COMPACT);
		json_decref(root);
	}
    ...
    while (aptMainLoop()) {
		...
		if (cpu_debug) {
			...
			C2D_DrawLine(cpuStartX, cpuStartY, C2D_Color32(255, 255, 0, 255),
						 cpuStartX + cpuLineLength, cpuStartY, C2D_Color32(255, 255, 0, 255), 5.0f, 0);
			...
			C2D_DrawRectSolid(18, 22, 0.1f, 100, 14, C2D_Color32(255, 255, 255, 255));
			...
			C2D_DrawLine(cpuStartX, cpuStartY + 30, C2D_Color32(255, 255, 0, 255),
						 cpuStartX + drawLineLength, cpuStartY + 30, C2D_Color32(255, 255, 0, 255), 5.0f, 0);
			...
			C2D_DrawRectSolid(18, 52, 0.1f, 100, 14, C2D_Color32(255, 255, 255, 255));
