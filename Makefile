...
#---------------------------------------------------------------------------------
APP_VER				:= 2048
APP_TITLE			:= Citrus Wallet 
APP_DESCRIPTION		:= Your versatile QR Code/Barcode Wallet!
APP_AUTHOR			:= TehFridge
PRODUCT_CODE		:= CTRW3DS
UNIQUE_ID			:= 0x19DE2

BANNER_AUDIO		:= meta/banner.wav
BANNER_IMAGE		:= meta/banner.png
ICON				:= meta/icon.png
RSF_PATH			:= meta/wallet.rsf
...
CFLAGS  := $(DEBUGFLAGS) -Wall -mword-relocations -ffunction-sections $(ARCH)
...
# NOTE: keep -g in LDFLAGS, and drop --gc-sections while debugging
LDFLAGS = -specs=3dsx.specs -g $(ARCH) -Wl,-Map,$(notdir $*.map)


LIBS := `curl-config --libs` -lcitro2d -lcitro3d -lctru `$(PREFIX)pkg-config vorbisidec --libs` -lcwav -lncsnd -ljansson -lmbedtls -lmbedcrypto -lzint -lm
